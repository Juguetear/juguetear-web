...
2. Una vez guardado el archivo, si tenés el server ya iniciado, vas a tener que reiniciarlo. Para eso, tenés que ir a la consola de comandos, apretar ctrl + C, y luego ejecutar `yarn run dev` o directamente `yarn dev` que en escencia, hacen lo mismo.
